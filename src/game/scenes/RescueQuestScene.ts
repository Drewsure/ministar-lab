...
export default class RescueQuestScene extends BaseEngine {
  ...
  protected buildWorld() {
    ...
    this.tweens.add({ targets: this.target, scale: { from: 1, to: 1.15 }, duration: 800, yoyo: true, repeat: 50, ease: 'Sine.inOut' });
  ...
  private updateCurrentObstacle() {
    ...
    this.obstacles.forEach((o, i) => {
      ...
      if (i === this.currentObstacleIdx) this.tweens.add({ targets: o.text, scale: { from: 1, to: 1.2 }, duration: 500, yoyo: true, repeat: 50, ease: 'Sine.inOut' });
    });

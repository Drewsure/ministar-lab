...
// ============================================================================
// RESCUE QUEST — Voice-activate actions to clear paths and save characters
// ============================================================================
// A character is trapped behind obstacles. Student must SAY action words
// ("Jump", "Dig", "Climb", "Swim", "Fly") to clear each obstacle and
// rescue the character. Builds verb vocabulary + pronunciation.
// ============================================================================

interface Obstacle {
  type: string;      // "wall" | "river" | "cliff" | "cage"
  command: string;   // the verb to say
  emoji: string;
  x: number;
  y: number;
  cleared: boolean;
...
export default class RescueQuestScene extends BaseEngine {
  ...
  protected buildWorld() {
    ...
    this.hero = this.add.text(this.heroX, this.heroY, '🦸', { fontSize: '48px' }).setOrigin(0.5).setDepth(100);

    // Target character to rescue (right side)
    this.target = this.add.text(this.scale.width - 80, this.heroY, '🐰', { fontSize: '48px' }).setOrigin(0.5).setDepth(100);
    // Pulse the target to show it needs rescue
    this.tweens.add({
      targets: this.target, scale: { from: 1, to: 1.15 }, duration: 800, yoyo: true, repeat: -1, ease: 'Sine.inOut',
    });
    ...
    // Create obstacles between hero and target
  ...
  private createObstacles() {
    const obstacleCount = this.maxScore;
    const startX = 180;
    const gap = (this.scale.width - 260) / obstacleCount;
  ...
  private updateCurrentObstacle() {
    if (this.currentObstacleIdx >= this.obstacles.length) {
      // All obstacles cleared — rescue complete!
    ...
    // Highlight current obstacle
    this.obstacles.forEach((o, i) => {
      o.label.setAlpha(i === this.currentObstacleIdx ? 1 : 0);
      if (i === this.currentObstacleIdx) {
        this.tweens.add({
          targets: o.text, scale: { from: 1, to: 1.2 }, duration: 500, yoyo: true, repeat: -1, ease: 'Sine.inOut',
        });
      }
    });
  ...
  private createMicButton() {
    ...
    const micBg = this.add.circle(0, 0, 30, this.theme.accent, 0.9)
  ...
  private initSpeechRecognition() {
    try {
      ...
      if (!SpeechRecognition) {
        this.statusText.setText('Speech recognition not available');
        return;
      }
  ...
  private startListening() {
    ...
    try {
      ...
      this.statusText.setText('Mic error: ' + e.message);
    }
  }
  ...
  private checkCommand(heard: string) {
    ...
    this.recordAnswer({
      ...
      response: heard,
      success: isCorrect,
      coordinate: { x: obs.x, y: obs.y, t: this.time.now },
    });

    if (isCorrect) {
      this.statusText.setText(`✓ ${obs.command}! Obstacle cleared!`);
      this.statusText.setColor(this.hex(this.theme.success));
      this.clearObstacle(obs);
      audioBus.speak(obs.command);
    } else {
      this.statusText.setText(`✗ You said "${heard}". Say "${obs.command}"`);
  ...
  private clearObstacle(obs: Obstacle) {
    obs.cleared = true;
    // Animate obstacle disappearing
    this.tweens.add({
      targets: [obs.text, obs.label],
      alpha: 0, scale: 0, y: '-=30',

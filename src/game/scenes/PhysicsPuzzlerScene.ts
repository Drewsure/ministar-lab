...
// Projectile movement uses Phaser's update loop (onTick), NOT requestAnimationFrame.
...
interface ProjectileData { obj: Phaser.GameObjects.Arc; vx: number; vy: number; bornAt: number; }
...
export default class PhysicsPuzzlerScene extends BaseEngine {
  ...
  protected buildWorld() {
    ...
    this.setupGlobalPointer((x, y) => { if (y < this.scale.height - 100 && this.canFire && !this.isFinished) this.fireProjectile(x, y); });
  ...
  protected onTick(_remainingMs: number) {
    ...
    // Update blocks (floating word movement)
    ...
    // Update projectiles (movement + gravity + lifetime) — uses Phaser update loop, NOT rAF
    const now = this.time.now;
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      const pd = this.projectiles[i];
      if (!pd.obj.active) {
        this.projectiles.splice(i, 1);
        continue;
      }
      // Lifetime expiry (3 seconds)
      if (now - pd.bornAt > 3000) {
        try { pd.obj.destroy(); } catch {}
        this.projectiles.splice(i, 1);
        continue;
      }
      // Apply gravity + movement
      pd.vy += 200 * 0.016;
      pd.obj.x += pd.vx * 0.016;
      pd.obj.y += pd.vy * 0.016;
      // Trail particle (spawn via Phaser timer, not rAF)
      if (Math.random() > 0.5) {
        const trail = this.add.circle(pd.obj.x, pd.obj.y, 4, this.theme.warning, 0.5).setDepth(89);
        this.tweens.add({ targets: trail, alpha: 0, scale: 0, duration: 300, onComplete: () => { try { trail.destroy(); } catch {} } });
      }
      // Collision check
      for (const b of this.blocks) {
        if (!b.container.active) continue;
        if (Phaser.Math.Distance.Between(pd.obj.x, pd.obj.y, b.container.x, b.container.y) < 50) {
          this.handleHit(b, pd.obj);
          this.projectiles.splice(i, 1);
          break;
        }
      }
  ...
  private renderRound() {
    ...
    this.blocks.forEach(b => { try { b.container.destroy(); } catch {} }); this.blocks = [];
    ...
    pool.slice(0, blockCount).forEach((term, i) => {
      ...
      this.blocks.push({ text: txt, container, term, isCorrect: term.id === promptId, vx: (Math.random() - 0.5) * 80, vy: (Math.random() - 0.5) * 60 });
    });
    this.canFire = true;
  }

  private fireProjectile(targetX: number, targetY: number) {
    if (this.shotsLeft <= 0 || this.isFinished || !this.canFire) return;
    ...
    this.projectiles.push({ obj: proj, vx, vy, bornAt: this.time.now });
  ...
  private handleHit(block: WordBlock, proj: Phaser.GameObjects.Arc) {
    ...
    try { block.container.destroy(); } catch {}
    if (!this.currentPrompt) return;
    const promptTerm = this.currentPrompt.term;
    this.recordAnswer({ term: promptTerm, response: block.term.term, success: block.isCorrect,

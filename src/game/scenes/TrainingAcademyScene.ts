...
// ============================================================================
// TRAINING ACADEMY — Simon Says with speech-to-text commands
// ============================================================================
// The mascot gives a command ("Jump!", "Spin!", "Dance!"). Student must
// SAY the command into the microphone. If the speech recognition matches,
// the mascot performs the action. Builds pronunciation + listening skills.
// ============================================================================

interface TrainingCommand {
  text: string;
  emoji: string;
  action: 'jump' | 'spin' | 'dance' | 'wave' | 'sit' | 'run';
}
...
export default class TrainingAcademyScene extends BaseEngine {
  ...
  protected buildWorld() {
    ...
    // Command display (big, center-top)
    ...
    this.makeSpeakable(this.commandText, 'Tap to hear the command');

    // Mascot (center)
    this.mascot = this.add.text(this.scale.width / 2, this.scale.height / 2, '🐶', {
      fontSize: '96px',
    }).setOrigin(0.5).setDepth(50);
    ...
    // Heard text (what speech recognition heard)
  ...
  private createMicButton() {
    ...
    const micBg = this.add.circle(0, 0, 35, this.theme.accent, 0.9)
  ...
  private initSpeechRecognition() {
    try {
      ...
      if (!SpeechRecognition) {
        this.statusText.setText('Speech recognition not available in this browser');
      ...
      this.recognition.onerror = (event: any) => {
        ...
        this.statusText.setText('Mic error: ' + (event.error ?? 'unknown'));
        this.statusText.setColor(this.hex(this.theme.danger));
      };
  ...
  private startListening() {
    ...
    try {
      ...
      this.statusText.setText('Listening... Say the command!');
      this.statusText.setColor(this.hex(this.theme.warning));
      this.heardText.setText('');
    } catch (e: any) {
      this.statusText.setText('Could not start mic: ' + e.message);
    }
  }
  ...
  private nextCommand() {
    ...
    // Speak the command so student hears it
  ...
  private checkCommand(heard: string) {
    ...
    const isCorrect = heardClean === target.toLowerCase() || heardClean.includes(target) || target.includes(heardClean);

    this.recordAnswer({
      term: this.currentCommand.text,
      response: heard,
      success: isCorrect,
    ...
    if (isCorrect) {
      this.statusText.setText('✓ Perfect! Watch the mascot!');
      this.statusText.setColor(this.hex(this.theme.success));
      this.animateMascot(this.currentCommand.action);
      audioBus.speak(this.currentCommand.text);
    } else {
      this.statusText.setText(`✗ You said "${heard}". The command was "${this.currentCommand.text}"`);
      this.statusText.setColor(this.hex(this.theme.danger));
      audioBus.speak(this.currentCommand.text);
    }
  ...
  private animateMascot(action: string) {
    ...
    switch (action) {
      case 'jump':
        this.tweens.add({ targets: m, y: '-=80', duration: 300, yoyo: true, repeat: 2, ease: 'Quad.out' });
        break;
      case 'spin':
        this.tweens.add({ targets: m, angle: 360, duration: 800, ease: 'Cubic.out' });
        break;
      case 'dance':
        this.tweens.add({ targets: m, x: '+=20', duration: 200, yoyo: true, repeat: 5, ease: 'Sine.inOut' });
        this.tweens.add({ targets: m, angle: 15, duration: 200, yoyo: true, repeat: 5 });
        break;
      case 'wave':
        this.tweens.add({ targets: m, angle: -20, duration: 200, yoyo: true, repeat: 3 });
        break;
      case 'sit':
        this.tweens.add({ targets: m, scaleY: 0.7, duration: 400, yoyo: true, ease: 'Cubic.out' });
        break;
      case 'run':
        this.tweens.add({ targets: m, x: '+=100', duration: 300, yoyo: true, ease: 'Cubic.out' });
        break;
    }
  }
}

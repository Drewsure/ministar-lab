#!/bin/bash
# verify-aaaa-features.sh 窶・Pre-Deployment Verification
...
check() {
  ...
  if [ ! -f "$file" ]; then
    echo "笶・FAIL: $name 窶・file missing: $file"; FAIL=$((FAIL+1)); FAILURES="$FAILURES\n  - $name (file missing)"; return
  fi
  count=$(grep -cE "$pattern" "$file" 2>/dev/null || true)
  [ -z "$count" ] && count=0
  if [ "$count" -ge "$min" ]; then
    echo "笨・PASS: $name ($count in $(basename $file))"; PASS=$((PASS+1))
  else
    echo "笶・FAIL: $name 窶・expected $min+, got $count"; FAIL=$((FAIL+1)); FAILURES="$FAILURES\n  - $name (expected $min+, got $count in $file)"
  fi
}
...
check_absent() {
  ...
  [ -z "$count" ] && count=0
  if [ "$count" -eq 0 ]; then
    echo "笨・PASS: $name (absent as expected)"; PASS=$((PASS+1))
  else
    echo "笶・FAIL: $name 窶・should NOT exist, found $count"; FAIL=$((FAIL+1)); FAILURES="$FAILURES\n  - $name (found $count, should be 0)"
  fi
}
...
if [ "$SG" -lt 32 ]; then echo "笶・FAIL: speakPromptWithHighlight $SG/32"; FAIL=$((FAIL+1)); else echo "笨・PASS: speakPromptWithHighlight $SG/32"; PASS=$((PASS+1)); fi
if [ "$HG" -lt 32 ]; then echo "笶・FAIL: makeHoverSpeakable $HG/32"; FAIL=$((FAIL+1)); else echo "笨・PASS: makeHoverSpeakable $HG/32"; PASS=$((PASS+1)); fi
...
if [ "$FAIL" -gt 0 ]; then
  echo "笶・DEPLOYMENT BLOCKED 窶・$FAIL feature(s) missing:"
  echo -e "$FAILURES"
  exit 1
else
  echo "笨・ALL CHECKS PASSED 窶・safe to deploy."
  exit 0
fi

import type { GameModeId, GameModeMeta } from './types';

// The 11 game modes — driven by three parent engines.
// Each engine reads the same unified TermItem[] dataset.

export const GAME_MODES: GameModeMeta[] = [
  // Selection Engine — physics collisions + distinct choice logic
  {
    id: 'maze-chase',
    name: 'Maze Chase',
    engine: 'Selection',
    description: 'Navigate a procedural maze, collect correct answers, dodge decoys.',
    emoji: '🧭',
    minTerms: 2,
    maxTerms: 12,
    accent: '#22d3ee',
  },
  {
    id: 'quiz',
    name: 'Quiz',
    engine: 'Selection',
    description: 'Multiple-choice quiz with timed answers and streak multipliers.',
    emoji: '❓',
    minTerms: 2,
    maxTerms: 20,
    accent: '#7c3aed',
  },
  {
    id: 'airplane',
    name: 'Airplane',
    engine: 'Selection',
    description: 'Fly through clouds and steer into the correct term banner.',
    emoji: '✈️',
    minTerms: 2,
    maxTerms: 10,
    accent: '#0ea5e9',
  },
  {
    id: 'gameshow',
    name: 'Gameshow Quiz',
    engine: 'Selection',
    description: 'Three-lifeline gameshow with spotlight reveal and confetti finale.',
    emoji: '🎯',
    minTerms: 2,
    maxTerms: 15,
    accent: '#c026d3',
  },
  // Pairing Engine — ID_A ↔ ID_B comparison
  {
    id: 'memory-match',
    name: 'Memory Match',
    engine: 'Pairing',
    description: 'Flip cards to find matching term pairs against the clock.',
    emoji: '🃏',
    minTerms: 2,
    maxTerms: 8,
    accent: '#22c55e',
  },
  {
    id: 'match-up',
    name: 'Match Up',
    engine: 'Pairing',
    description: 'Drag left-column terms to right-column definitions.',
    emoji: '🔗',
    minTerms: 3,
    maxTerms: 10,
    accent: '#15803d',
  },
  {
    id: 'balloon-pop',
    name: 'Balloon Pop',
    engine: 'Pairing',
    description: 'Pop the balloon carrying the term that matches the prompt.',
    emoji: '🎈',
    minTerms: 2,
    maxTerms: 12,
    accent: '#fb7185',
  },
  {
    id: 'whack-a-mole',
    name: 'Whack-a-Mole',
    engine: 'Pairing',
    description: 'Bonk the mole surfacing with the correct answer.',
    emoji: '🔨',
    minTerms: 2,
    maxTerms: 10,
    accent: '#a16207',
  },
  // Text / Spelling Engine — array sequencing + letter deduction
  {
    id: 'anagram',
    name: 'Anagram',
    engine: 'Text',
    description: 'Unscramble letters to spell each vocabulary word.',
    emoji: '🔤',
    minTerms: 2,
    maxTerms: 15,
    accent: '#fbbf24',
  },
  {
    id: 'wordsearch',
    name: 'Word Search',
    engine: 'Text',
    description: 'Find vocabulary words hidden in a generated letter grid.',
    emoji: '🔍',
    minTerms: 3,
    maxTerms: 12,
    accent: '#8b5cf6',
  },
  {
    id: 'bridge-builder',
    name: 'Bridge Builder',
    engine: 'Text',
    description: 'Rocket Launch mechanic — guess letters to span the river. (Hangman alternative.)',
    emoji: '🚀',
    minTerms: 1,
    maxTerms: 20,
    accent: '#f97316',
  },
  {
    id: 'crossword',
    name: 'Crossword',
    engine: 'Text',
    description: 'AI-generated dense crossword grid from your term list. Tap cells, type answers.',
    emoji: '🔤',
    minTerms: 3,
    maxTerms: 12,
    accent: '#10b981',
  },
  {
    id: 'flash-cards',
    name: 'Flash Cards',
    engine: 'Pairing',
    description: 'Swipe through digital flash cards. Tap to flip, hear pronunciation, mark known/unknown.',
    emoji: '📇',
    minTerms: 2,
    maxTerms: 30,
    accent: '#a78bfa',
  },
  {
    id: 'spin-wheel',
    name: 'Spin Wheel',
    engine: 'Selection',
    description: 'Spin the wheel of terms. Wherever it lands, speak the word and answer the prompt.',
    emoji: '🎡',
    minTerms: 3,
    maxTerms: 12,
    accent: '#f97316',
  },
  {
    id: 'group-sort',
    name: 'Group Sort',
    engine: 'Pairing',
    description: 'Sort terms into categories. Drag each term to the correct group bucket.',
    emoji: '🗃️',
    minTerms: 4,
    maxTerms: 16,
    accent: '#06b6d4',
  },
  {
    id: 'type-answer',
    name: 'Type Answer',
    engine: 'Text',
    description: 'Read the definition, type the correct term. Builds spelling + recall skills.',
    emoji: '⌨️',
    minTerms: 2,
    maxTerms: 20,
    accent: '#8b5cf6',
  },
  {
    id: 'spot-it',
    name: 'Spot It!',
    engine: 'Selection',
    description: 'Find the matching symbol between two cards! Fast eyes win! Based on the hit card game Dobble.',
    emoji: '👁️',
    minTerms: 3,
    maxTerms: 20,
    accent: '#f43f5e',
  },
  // AAAA — Arcade Games (reinforcement tools)
  {
    id: 'label-it',
    name: 'Label It',
    engine: 'Pairing',
    description: 'Drag labels onto hotspots on a teacher-uploaded diagram. Great for science & maps.',
    emoji: '🏷️',
    minTerms: 2,
    maxTerms: 10,
    accent: '#06b6d4',
  },
  {
    id: 'speak-it',
    name: 'Speak It',
    engine: 'Selection',
    description: 'SAY the word that matches the definition. Uses speech recognition for pronunciation practice.',
    emoji: '🗣️',
    minTerms: 2,
    maxTerms: 10,
    accent: '#ec4899',
  },
  {
    id: 'endless-runner',
    name: 'Endless Runner',
    engine: 'Selection',
    description: 'Switch lanes to hit the correct word. Speed increases — how far can you run?',
    emoji: '🏃',
    minTerms: 2,
    maxTerms: 15,
    accent: '#f97316',
  },
  {
    id: 'physics-puzzler',
    name: 'Physics Puzzler',
    engine: 'Selection',
    description: 'Aim and fire projectiles at the correct floating word blocks.',
    emoji: '🎯',
    minTerms: 2,
    maxTerms: 10,
    accent: '#8b5cf6',
  },
  {
    id: 'snaking',
    name: 'Word Snake',
    engine: 'Selection',
    description: 'Guide the snake to eat correct words and grow. Avoid wrong words and walls!',
    emoji: '🐍',
    minTerms: 2,
    maxTerms: 10,
    accent: '#22c55e',
  },
  {
    id: 'training-academy',
    name: 'Training Academy',
    engine: 'Selection',
    description: 'Say action commands (Jump! Spin! Dance!) to make the mascot move. Speech-to-text.',
    emoji: '🎓',
    minTerms: 2,
    maxTerms: 10,
    accent: '#fbbf24',
  },
  {
    id: 'rescue-quest',
    name: 'Rescue Quest',
    engine: 'Selection',
    description: 'Say action verbs to clear obstacles and rescue trapped characters. Voice-activated.',
    emoji: '🦸',
    minTerms: 2,
    maxTerms: 6,
    accent: '#ef4444',
  },
  {
    id: 'farm-life',
    name: 'Farm Life',
    engine: 'Selection',
    description: 'Plant, water, and harvest crops on your farm. Learn food and nature vocabulary!',
    emoji: '🌾',
    minTerms: 8,
    maxTerms: 12,
    accent: '#22c55e',
  },
  {
    id: 'monster-fighter',
    name: 'Monster Fighter',
    engine: 'Selection',
    description: 'Battle monsters by casting spells! Match the weakness for double damage. RPG combat!',
    emoji: '⚔️',
    minTerms: 8,
    maxTerms: 12,
    accent: '#e94560',
  },
  {
    id: 'tower-defense',
    name: 'Tower Defense',
    engine: 'Selection',
    description: 'Defend your castle! Tap the correct word to shoot enemies before they reach your wall!',
    emoji: '🏰',
    minTerms: 8,
    maxTerms: 12,
    accent: '#533483',
  },
  {
    id: 'rhythm-tap',
    name: 'Rhythm Tap',
    engine: 'Selection',
    description: 'Tap vocabulary words as they fall to the beat! Perfect timing = bonus points!',
    emoji: '🎵',
    minTerms: 8,
    maxTerms: 12,
    accent: '#a78bfa',
  },
  {
    id: 'space-explorer',
    name: 'Space Explorer',
    engine: 'Selection',
    description: 'Explore star systems and discover planets by matching vocabulary words!',
    emoji: '🚀',
    minTerms: 8,
    maxTerms: 12,
    accent: '#60a5fa',
  },
  {
    id: 'story-adventure',
    name: 'Story Adventure',
    engine: 'Selection',
    description: 'Live an interactive story! Choose what happens next and learn vocabulary through adventure!',
    emoji: '📖',
    minTerms: 8,
    maxTerms: 12,
    accent: '#fbbf24',
  },
];

export function getGameMode(id: GameModeId): GameModeMeta {
  return GAME_MODES.find((m) => m.id === id) ?? GAME_MODES[0];
}

export const GAME_MODE_MAP: Record<GameModeId, GameModeMeta> = Object.fromEntries(
  GAME_MODES.map((m) => [m.id, m])
) as Record<GameModeId, GameModeMeta>;
